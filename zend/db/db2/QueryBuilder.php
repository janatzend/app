...
class QueryBuilder extends \yii\db\QueryBuilder
{
    ...
    public function buildLimit($limit, $offset)
    {
        ...
        $limitOffsetStatment = 'SELECT * FROM (SELECT SUBQUERY_.*, ROW_NUMBER() OVER() AS RN_ FROM ( :query ) AS SUBQUERY_) AS t WHERE t.RN_ BETWEEN :offset AND :limit';

        $replacement = $this->hasOffset($offset) ? $offset : '0';
        $limitOffsetStatment = str_replace(':offset', $replacement, $limitOffsetStatment);

        $replacement = $this->hasLimit($limit) ? ($limit + $offset) : '999999999';
